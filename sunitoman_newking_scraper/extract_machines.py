...
# 機種スペック（SBニューキングハナハナV-30）
COIN_PER_GAME = 1.4124  # ベース約35.4G (取りこぼし考慮の実ベース) = 約1.4124枚/G
BIG_COINS = 312
REG_COINS = 130

# 設定確率 (設定1〜4, V)
SETTINGS = ["設定1", "設定2", "設定3", "設定4", "設定V"]
PROB_BIG = [1/299, 1/291, 1/281, 1/268, 1/253]
PROB_REG = [1/496, 1/471, 1/442, 1/409, 1/372]

PRIOR_PROB = [0.45, 0.20, 0.15, 0.10, 0.10] # ホール側の設定配分（仮定）
    ...
    for row in rows:
        ...
        if len(cols) >= 11:
            try:
                ...
                games = int(cols[6].get_text(strip=True))
