...
if data:
    ...
    is_tokubi = tomorrow.day % 10 in [0, 1, 2]

    tomorrow_tags = []
    if is_friday: tomorrow_tags.append("華金（金曜日）")
    if is_tokubi: tomorrow_tags.append("特日（0・1・2の付く日）")
...
for daiban, history in machine_history.items():
    ...
    # 暫定ロジック（A案＋B案の融合）
    # サンシャイン糸満店の癖（据え置き率0%、平均高設定投入率6%、大凹み上げ率11%）に特化
    
    # 1週間ベースでの飢餓状態・凹み評価 (A案)
    if diff_7days <= -4000:
        score += 80
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .score-bar-fill {{ height: 100%; background: linear-gradient(to right, #ff4757, #ffa502); box-shadow: 0 0 10px rgba(255, 71, 87, 0.5); }}
        ...
        .prob-6 {{ background: #ff4757; }}
        .prob-5 {{ background: #ff6b81; }}
        .prob-4 {{ background: #ffa502; }}
        .prob-3 {{ background: #eccc68; }}
        .prob-2 {{ background: #7bed9f; }}
        .prob-1 {{ background: #747d8c; }}
...
<body>
    <div class="container">
        ...
        <header>
            ...
            <p class="subtitle">Sドラゴンハナハナ-閃光-SP-30 / サンシャイン糸満店 特化システム</p>
        </header>
    ...
    <script>
        ...
        function selectDate(day, btnElement) {{
            ...
            if(overallRegProb > 0) {{
                if(overallRegProb <= 399) hallSetting = "設定6 (極甘)";
                else if(overallRegProb <= 442) hallSetting = "設定5 (甘)";
                else if(overallRegProb <= 489) hallSetting = "設定4 (普通)";
                else if(overallRegProb <= 537) hallSetting = "設定3 (辛)";
                else if(overallRegProb <= 585) hallSetting = "設定2 (激辛)";
            }}
        ...
        function renderTable(data) {{
            ...
            data.forEach(row => {{
                ...
                else if (setting.includes('4') || setting.includes('3')) settingClass = 'setting-ok';
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <div style="font-size: 0.75rem; display: flex; justify-content: space-between; width: 120px;">
                            <span style="color:#ff4757">6:${{Math.round(p6)}}%</span>
